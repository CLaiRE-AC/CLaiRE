...
export default class Invitation extends Command {
  ...
  static flags = {
    list: Flags.boolean({ description: 'List all invitations' }),
    show: Flags.string({ description: 'Show a specific invitation by ID' }),
    create: Flags.boolean({ description: 'Create a new invitation' }),
  ...
  async run() {
    ...
    if (flags.show) {
      const response = await axios.get(`${API_URL}/invitations/${flags.show}`, {
      	headers: { Authorization: `Bearer ${authToken}` }
      });
      this.log(JSON.stringify(response.data, null, 2));
      return;
